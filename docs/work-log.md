...
### 결과 검증 (production 첫 회차, 2026-05-03 KST 13:53)

- HTTP 200, duration **588초** (한도 600 의 98%, 마진 12초)
- memory 935MB / 2048MB (여유)
- 미러 학교 수 **604 / 654** (사진 가능 654교 중 92.4%)
  - SQL: `select count(distinct school_id) from meal_photos;` → 604
  - 미러 안 들어온 ~50교 = 이 회차 weekMap 빈 결과. 학교 서버 일시 응답 지연 /
    영양교사 업로드 timing. 다음 회차 (14:30 / 17:00 / 다음날 08:00) 자동 재시도
    + sliding window 8일 보존이라 한두 회차 빈 결과여도 데이터 손실 0.

처방 효과 검증 완료 — 686 교 cron 이 한 invocation 으로 무사 진입.
다만 마진 12초로 빠듯 → Stage 14-28 안정성 보강 진행.

---

## Stage 14-28 — 685교 cron 안정성 마진 보강 (2026-05-03)

Stage 14-27 첫 회차 588s/600 한도 = 98% 사용. 학교 서버 응답 변동성으로 다시
504 가능. 마진 확보 처방 우선순위 결정:

후보:
- A. chunk 35 → 30. batch 수 20 → 23 으로 늘어 batch 안 wall 약간 단축. 가장 작은 변경.
- B. 학교당 다운로드 timeout 15s → 10s. 느린 학교 1~2 곳 정체 영향 최소화.
  사진 미업로드 24교 (school-status.md 참고) 의 일부가 사실 "느린 응답" 일 수도
  있어 timeout 단축이 미러 누락에 영향 줄 수도 있음 — 위험 약간 있음.
- C. cron 분리 (`refresh-neis` + `refresh-photos`). 가장 구조적이지만 vercel.json
  +라우트 추가 + 운영 검증 필요. 다음 사고 시 처방으로 보류.

결정: **A 만 적용** (사용자 선택). risk 가장 낮은 처방으로 안정성 우선. B/C 는
504 재발 시 추가 처방으로 보류.

변경: `src/app/api/cron/refresh/route.ts` 의 `CHUNK = 35` → `CHUNK = 30` + 주석 갱신.
